classdef BaseTools
    ...
    methods (Static)
        ...
        function fh = extractFigureHandle( args )
            fh = [];
            for k = 1 : length(args.posArgs)
                if isa( args.posArgs{k}, 'matlab.ui.Figure' )
                    fh = args.posArgs{k};
                end
            end
            if isempty(fh)
        ...
        function [ X, Y ] = getXYgrid( xrng, yrng, dx, dy )
            % Return a meshgrid of X, Y coordinates build to approximately
            % match the input ranges (xrng, yrng) but subject to the
            % desired spacing (dx,dy) and in such a way that the origin
            % would be included if it were in the range.
            x1 = ceil(min(xrng(:))/dx)*dx;
            x2 = floor(max(xrng(:))/dx)*dx;
            y1 = ceil(min(yrng(:))/dy)*dy;
            y2 = floor(max(yrng(:))/dy)*dy;
